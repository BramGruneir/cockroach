...
func (ef *execFactory) ConstructUpdate(
	...
	ru, err := row.MakeUpdater(tabDesc, updateColDescs, fetchColDescs, row.UpdaterDefault, fkChecker)
	if err != nil {
		return nil, err
	}
...
func (ef *execFactory) ConstructDelete(
	...
	rd, err := row.MakeDeleter(tabDesc, fetchColDescs, row.CheckFKs, fkChecker)
	if err != nil {
		return nil, err
	}
