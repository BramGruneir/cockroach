...
func makeFKDeleteHelper(
	...
	for _, idx := range table.AllNonDropIndexes() {
		for _, ref := range idx.ReferencedBy {
			...
			idxDesc, err := otherTables[ref.Table].Table.FindIndexByID(ref.Index)
			if err != nil {
				return fkDeleteHelper{}, err
			}
			if h.fks == nil {
...
// cascadeHelper is a struct that acts upon a single index of a single table. It
// contains all spans, row fetchers and row writers required for any cascading
// operation.
// TODO(bram): consider storing and retreiving the referencedRF so we don't need
// to recreate for circular dependencies.
type cascadeHelper struct {
	txn          *client.Txn
	table        *TableDescriptor // the table being searched for rows to cascade
	index        *IndexDescriptor // the fk index that will contain the entries to cascade
...
// delete performs the actual delete of the rows. This call may be recursive via
// the rowDeleter.
