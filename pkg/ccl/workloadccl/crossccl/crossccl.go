...
`
	...
	customersSchema = `
(
  session_id STRING NOT NULL REFERENCES sessions ON DELETE CASCADE
 ,id STRING NOT NULL
 ,a STRING NOT NULL
 ,created TIMESTAMP NOT NULL
 ,updated TIMESTAMP NOT NULL
...
`
	...
	defaultDevicesPerSession    = 2
	defaultVariantsPerSession   = 5
	defaultParametersPerSession = 2
	...
	return workload.Hooks{
		PreLoad: func(db *gosql.DB) error {
			...
			if _, err := db.Exec(
				...
				//"ALTER PARTITION west OF TABLE sessions EXPERIMENTAL CONFIGURE ZONE 'experimental_lease_preferences: [[+rack=0]]'",
			); err != nil {
			...
			if _, err := db.Exec(
				...
				//"ALTER PARTITION east OF TABLE sessions EXPERIMENTAL CONFIGURE ZONE 'experimental_lease_preferences: [[+rack=1]]'",
			); err != nil {
...
func (c *crossccl) Tables() []workload.Table {
	...
	sessionIDs := make([]string, c.sessions)
	for i := 0; i < c.sessions; i++ {
		sessionIDs[i] = c.createRandomSessionID(rng, rng.Intn(100) < c.eastPercent)
	}
	...
	customers := workload.Table{
		...
		InitialRows: workload.Tuples(
			c.sessions*c.customersPerSession,
			func(rowIdx int) []interface{} {
				rng := rand.New(rand.NewSource(time.Now().UnixNano()))
				return []interface{}{
					sessionIDs[rowIdx/c.customersPerSession], // session_id
	...
	devices := workload.Table{
		...
		InitialRows: workload.Tuples(
			c.sessions*c.devicesPerSession,
			func(rowIdx int) []interface{} {
				rng := rand.New(rand.NewSource(time.Now().UnixNano()))
				return []interface{}{
					sessionIDs[rowIdx/c.devicesPerSession], // session_id
					fmt.Sprint(rowIdx),                     // id
					string(randutil.RandBytes(rng, 50)),    // a
					string(randutil.RandBytes(rng, 50)),    // b
	...
	variants := workload.Table{
		...
		InitialRows: workload.Tuples(
			c.sessions*c.variantsPerSession,
			func(rowIdx int) []interface{} {
				rng := rand.New(rand.NewSource(time.Now().UnixNano()))
				return []interface{}{
					sessionIDs[rowIdx/c.variantsPerSession], // session_id
					fmt.Sprint(rowIdx),                      // id
					string(randutil.RandBytes(rng, 50)),     // a
					timeutil.Now(),                          // date_created
					timeutil.Now(),                          // date_updated
	...
	parameters := workload.Table{
		...
		InitialRows: workload.Tuples(
			c.sessions*c.parametersPerSession,
			func(rowIdx int) []interface{} {
				rng := rand.New(rand.NewSource(time.Now().UnixNano()))
				return []interface{}{
					sessionIDs[rowIdx/c.parametersPerSession], // session_id
					fmt.Sprint(rowIdx),                        // id
					string(randutil.RandBytes(rng, 50)),       // a
					timeutil.Now(),                            // date_created
					timeutil.Now(),                            // date_updated
...
const insertQueryDevices = `INSERT INTO devices VALUES ($1, $2, $3, $4, $5, $6, $7, $8, now(), now())`
const insertQueryVariants = `INSERT INTO variants VALUES ($1, $2, $3, now(), now())`
const insertQueryParameters = `INSERT INTO parameters VALUES ($1, $2, $3, now(), now())`
	...
	for i := 0; i < c.connFlags.Concurrency; i++ {
		...
		workerFn := func(ctx context.Context) error {
			...
			if opRand < c.insertPercent {
				...
				// Devices
				for j := 0; j < c.devicesPerSession; j++ {
					start := timeutil.Now()
				...
				// Customers
				for j := 0; j < c.customersPerSession; j++ {
					start := timeutil.Now()
				...
				// Variants
				for j := 0; j < c.variantsPerSession; j++ {
					start := timeutil.Now()
				...
				// Parameters
				for j := 0; j < c.parametersPerSession; j++ {
					start := timeutil.Now()
